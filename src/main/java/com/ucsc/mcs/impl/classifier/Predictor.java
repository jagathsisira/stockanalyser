...
public class Predictor {
    ...
    private ArrayList<NewsData> newsSamples = new ArrayList<>();

    private ArrayList<AnnouncementData> announcementsSamples = new ArrayList<>();
    ...
    private void predict() {
        loadItemsToPredict();
        predictText(null, null);
    }

    private void loadItemsToPredict() {
        Connection dbConnection = MySqlConnector.getInstance().connect();
        int count = 0;
        ...
        System.out.println("Anns Samples count " + announcementsSamples.size());
        ...
        System.out.println("News Samples count " + newsSamples.size());
    }

    public void predictText(ArrayList<AnnouncementData> announcementsList, ArrayList<NewsData> newsList) {

        if(announcementsList != null && !announcementsList.isEmpty()){
            this.announcementsSamples = announcementsList;
        }

        if(newsList != null && !newsList.isEmpty()){
            this.newsSamples = newsList;
        }
        /**
...
//        textClassifier.save("SentimentAnalysis");
        ...
        for(NewsData newsData : newsSamples) {
        ...
        for(AnnouncementData announcementData : announcementsSamples) {
            //Classify a single sentence
